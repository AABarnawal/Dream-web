  ...
  const TimelineSection = () => {
    ...
    return (
      ...
      >
        ...
        <VerticalTimeline lineColor="var(--color-gray-200)">
          {serviceTimeline.map((step, index) => (
            <VerticalTimelineElement
              key={step.step}
              className="vertical-timeline-element--work"
              contentStyle={{
                background: index % 2 === 0 ? "var(--color-blue-50)" : "var(--color-red-50)",
                color: "#fff",
                boxShadow: "0 3px 0 var(--color-gray-200)",
              }}
              contentArrowStyle={{ borderRight: "7px solid var(--color-gray-200)" }}
              date={`Step ${step.step}`}
              iconStyle={{
                background: index % 2 === 0 ? "var(--color-blue-500)" : "var(--color-red-500)",
                color: "#fff",
                display: "flex",
                justifyContent: "center",
                alignItems: "center",
              }}
              icon={<span className="font-bold text-lg">{step.step}</span>}
            >
              <h3 className="vertical-timeline-element-title font-bold text-lg mb-2 text-gray-900 dark:text-white">
                {step.title}
              </h3>
